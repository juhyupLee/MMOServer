...
class CSharpGenerator : public BaseGenerator {
 ...
 private:
  ...
  std::string SourceCast(const Type& type,
    ...
    if (IsSeries(type)) {
      ...
      if (type.base_type == BASE_TYPE_UTYPE && type.enum_def != nullptr) {
        return "(" + GenTypeBasic(type.enum_def->underlying_type, false) +
               (isOptional ? "?" : "") + ")";
      }
      if (IsEnum(type))
  ...
  std::string GenMethod(const Type& type) const {
    // A union discriminator (UTYPE) must use the union enum's explicit
    // underlying scalar type.
    if (type.base_type == BASE_TYPE_UTYPE && type.enum_def != nullptr) {
      return ConvertCase(GenTypeBasic(type.enum_def->underlying_type, false),
                         Case::kUpperCamel);
    }
    return IsScalar(type.base_type)
