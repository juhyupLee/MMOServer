...
void NetworkSession::Transfer(char* messageBuffer, int32_t messageSize)
{
	//시간설정
	...
	//핑패킷은 무시
	if (messageID == MessageID::FClientPing || messageID == MessageID::FServerPing)
		return;

	//패킷로그
	if (LogManager::GetInstance()->GetPacketLogActivate())
	{
		LogManager::GetInstance()->PrintPacket(std::format("[session:{},key:{}]", m_sessionID, m_key), LogManager::MessageToJson(messageBuffer));
	}

	if (m_proxyCallback != nullptr && packedMessageHolder->accountID() != nullptr)
	{
		std::vector<AccountID> accountIDs{ };
		for (auto iter = packedMessageHolder->accountID()->begin(); iter != packedMessageHolder->accountID()->end(); ++iter)
		{
			accountIDs.emplace_back(*iter);
		}

		if (m_proxyCallback(m_key, accountIDs, messageID, (uint8_t*)messageBuffer, PACKET_HEADER_SIZE + messageSize) == true)
			return;
	}

	const auto messageHolder = MessageHolderPtr(packedMessageHolder->UnPack());
	if (messageID > MessageID::F_MEMBER_MESSAGE_MIN && messageID < MessageID::P_SERVER_MESSAGE_MAX)
	{
		m_callback(m_key, messageHolder);
	}
	else
	{
		m_callback((HANDLE)m_sessionID, messageHolder);
	}
}